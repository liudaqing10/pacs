...
*/
  ...
  set<Edge,clscmp> sed; 
  for (int i=0; i<3*vs; i++) { sed.insert(vedg[i]); };

  cout<<"number of edges = "<< sed.size() <<endl;
  cout<<"The edges are: "<<endl;
  for (auto i:sed) cout<<"("<<i.getvtc(0).getid()<<", "<<i.getvtc(0).getx()<<", "<<i.getvtc(0).gety()<<"), ("<<\
                       i.getvtc(1).getid()<<", "<<i.getvtc(1).getx()<<", "<<i.getvtc(1).gety()<<")"<<endl;
